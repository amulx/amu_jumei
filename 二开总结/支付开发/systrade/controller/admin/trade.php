...
class systrade_ctl_admin_trade extends desktop_controller{
     ...
     * 线下汇款订单处理
     */
    public function doReittance()
    {
        //var_dump('systrade_ctl_admin_trade.doReittance:' .json_encode($payment));
        $pagedata['payment'] = input::get('payment');
        return view::make('systrade/admin/trade/doreittance.html', $pagedata)->render();
    }

    public function saveReittance()
    {
        $payment = input::get('payment');
        $pay = input::get('pay');

        // 更新ectools相关表
        $reittance = new ectools_payment_reittance(app::get('ectools'));
        $para['payment'] = $payment;
        $para['pay'] = $pay;
        $data = array(
            'pay_type' => 'remittance',
            'pay_time' => time(),
            'payed_fee' => $pay['payed_fee'],
            'status' => WAIT_SELLER_SEND_GOODS,
        );
        //var_dump('saveReittance:' .json_encode($data));
        if ($reittance->paymentCompled($para))
        {
            $this->begin("?app=systrade&ctl=admin_trade&act=index");
            $data = array(
                'pay_type' => 'remittance',
            ...
            $filter = array('tid' => $payment['tid']);
            app::get('systrade')->model('trade')->update($data, $filter);
            $data = array(
                'pay_time' => time(),
                'status' => WAIT_SELLER_SEND_GOODS,
            );
            app::get('systrade')->model('order')->update($data, $filter);
            $this->end(true);
        }
    }
}
