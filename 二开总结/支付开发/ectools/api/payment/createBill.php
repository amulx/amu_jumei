...
class ectools_api_payment_createBill{
    ...
    public function create($params)
    {
        if(!$params['tids'])
        {
            throw new \LogicException(app::get('topc')->_("支付失败,没有要支付的订单"));
        }
        ...
        {
            throw new \LogicException(app::get('topc')->_("支付失败,支付的订单信息有误"));
        }

        $money = array_column($trades,'payment');
        if($params['money'] != array_sum($money))
        {
            throw new \LogicException(app::get('topc')->_("支付失败,需要支付的金额预订单金额不一致"));
        }
        ...
        try{
            ...
            if(!$result)
            {
                    throw new \LogicException(app::get('topc')->_("支付失败,支付单创建失败"));
            }
            ...
            foreach($trades as $value)
            {
                ...
                {
                    throw new \LogicException(app::get('topc')->_("支付失败,您有订单已经支付，请重新选择订单支付"));
                }
                $payBill = array(
                    'payment_id' => $paymentId,
                    'tid' => $value['tid'],
                ...
                if(!$billResult)
                {
                    throw new \LogicException(app::get('topc')->_("支付失败,支付单明细添加失败"));
                }
            }
            $db->commit();
        }
