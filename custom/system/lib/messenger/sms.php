...
class system_messenger_sms implements system_interface_messenger_send{
    ...
    {
        logger::info('[system_messenger_smg.send] : $content='.json_decode($content));
        ...
        */
        ...
        try{
            logger::info('[system_messenger_smg.send] : 1111111111-----------sendSMS');
            $result = kernel::single('system_messenger_smsemp')->sendSMS($content,$to);
            logger::info('[system_messenger_smg.send] : $result=' .json_encode($result));
            // $result = kernel::single('system_messenger_smschg')->send($contents,$config);
        }catch(Exception $e){
            logger::error('[system_messenger_smg.send] : $msg=' .$e->getMessage());
            $msg = $e->getMessage();
