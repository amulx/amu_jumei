...
class sysitem_data_item {
    ...
    {
        // echo "<pre>";var_dump($postData['spec']);exit();
        //商品上限判断
    ...
    {
        // echo "<pre>";var_dump(json_decode($postData['spec'],1));exit();
...
// echo "<pre>";var_dump($value);exit();
        ...
        if( $item['hash'] )
        {
            if( $this->__checkProductHash($item['hash'], $item['item_id'],$item['shop_id'],'item') )
            {
                $msg = app::get('sysitem')->_('您所填写的hash值已被使用，请检查！');
                throw new \LogicException($msg);
            }
        }
...
// echo "<pre>";var_dump($item);exit();
    ...
    private function __checkProductHash($hash, $itemId=0, $shopId=0,$type="item"){
        if(empty($hash)){
            return false;
        }
        $ojbMdlItem = app::get('sysitem')->model('item');
        if($type == "item")
        {
            $data = $ojbMdlItem->getRow("item_id",['hash'=>$hash,'shop_id'=>$shopId,'item_id|noequal'=>$itemId]);
            if($data) return true;
        }
        return false;
    }
